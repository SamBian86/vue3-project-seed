      ...
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              <!-- <el-button
                type="primary"
                v-if="filterPermission('sys:dataplatform:view')"
              ...
              <el-button
                type="primary"
                v-if="filterPermission('sys:dataplatform:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('SysDataplatform.areaDistribution')"
            min-width="160"
          >
            <template #default="scope">
              <div v-html="itemHandle(scope.row.areaDistribution)"></div>
            </template>
          </el-table-column>
          <el-table-column
            prop="residentAge"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.residentAge')"
            width="160"
          >
            <template #default="scope">
              <div v-html="itemHandle(scope.row.residentAge)"></div>
            </template>
          </el-table-column>
          <el-table-column
            prop="residentSex"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.residentSex')"
            width="160"
          >
            <template #default="scope">
              <div v-html="itemHandle(scope.row.residentSex)"></div>
            </template>
          </el-table-column>
          <el-table-column
            prop="deptNum"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.deptNum')"
            width="100"
          ></el-table-column>
          <el-table-column
            prop="doctorNum"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.doctorNum')"
            width="100"
          ></el-table-column>
          <el-table-column
            prop="registerNum"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.registerNum')"
            width="100"
          ></el-table-column>
          <el-table-column
            prop="dailyRegisterNum"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.dailyRegisterNum')"
            width="100"
          ></el-table-column>
          <el-table-column
            prop="visitNum"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.visitNum')"
            width="100"
          ></el-table-column>
          <el-table-column
            prop="visitCount"
            :show-overflow-tooltip="true"
            :label="$t('SysDataplatform.visitCount')"
            width="100"
          ></el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="120">
            <template #default="scope">
              ...
              <el-button
                type="text"
                v-if="filterPermission('sys:dataplatform:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('sys:dataplatform:delete')"
                :size="StyleEnum.BUTTON_SIZE"
...
// 表单页面
import SysDataplatformForm from './form.vue'
export default defineComponent({
  name: 'SysDataplatform', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, SysDataplatformForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deleteSysDataplatformByIdHandle } = useSysDataplatformRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deleteSysDataplatformByIdHandle(params, () => {
        // 模板修改标记
    ...
    // itemHandle
    function itemHandle(str: any) {
      const arr = JSON.parse(str)
      let _html = ''
      arr.forEach((item: any) => {
        _html += `<div>${item.name}:${item.value}</div>`
      })
      return _html
    }
