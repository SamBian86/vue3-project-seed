  ...
  <el-row :gutter="StyleEnum.ROW_GUTTER">
      ...
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
              ...
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="status">
                  <el-select
                    ...
                    :placeholder="$t('SysFrontUser.statusPlaceHolder')"
                    filterable
                    clearable
                  >
                    <el-option
                      v-for="item in getDictByType('frontUserStatus')"
                      :key="item.dictValue"
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              <el-button
                type="primary"
                v-if="filterPermission('sys:front:user:view')"
              ...
              <!-- <el-button
                type="primary"
                v-if="filterPermission('sys:front:user:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('SysFrontUser.username')"
            width="160"
          ></el-table-column>
          <el-table-column prop="tagNames" :show-overflow-tooltip="true" :label="$t('SysFrontUser.tagNames')"></el-table-column>
          <el-table-column
            prop="realName"
            :show-overflow-tooltip="true"
            :label="$t('SysFrontUser.realName')"
            min-width="160"
          ></el-table-column>
          <el-table-column
            prop="genderName"
            :show-overflow-tooltip="true"
            :label="$t('SysFrontUser.genderName')"
            width="100"
          ></el-table-column>
          <el-table-column prop="mobile" :show-overflow-tooltip="true" :label="$t('SysFrontUser.mobile')"></el-table-column>
          <el-table-column prop="status" :show-overflow-tooltip="true" :label="$t('SysFrontUser.status')" width="100">
            <template #default="scope">
              <el-tag v-if="scope.row.status === 0" :size="StyleEnum.TAG_SIZE" type="warngin">
                {{ getDictNameByValue('frontUserStatus', scope.row.status) }}
              </el-tag>
          ...
          <el-table-column
            prop="createDate"
            :show-overflow-tooltip="true"
            :label="$t('SysFrontUser.createDate')"
            width="160"
          ></el-table-column>
          <el-table-column
            prop="cancelDate"
            :show-overflow-tooltip="true"
            :label="$t('SysFrontUser.cancelDate')"
            width="160"
          ></el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="160">
            <template #default="scope">
              ...
              <!-- <el-button
                type="text"
                v-if="filterPermission('sys:front:user:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('sys:front:user:delete')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('sys:front:user:display') && scope.row.status !== 2"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
                  displaySysFrontUserHandle({ id: scope.row.id, status: scope.row.status === 1 ? 0 : 1 }, () => {
                    pgTableQuery(tableParams)
                  })
                "
              >
                {{ scope.row.status === 1 ? $t('SysFrontUser.off') : $t('SysFrontUser.on') }}
              </el-button>
              <el-button
                type="text"
                v-if="filterPermission('sys:front:user:view')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="tagHandle(scope.row)"
              >
                {{ $t('SysFrontUser.tag') }}
              </el-button>
            </template>
    ...
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <DialogPage ref="dialogPage">
        ...
        <template #title>
          {{ $t('SysFrontUser.pageTitle') }}
        </template>
      </DialogPage>
    </el-col>
  </el-row>
  <DrawerPage ref="drawerPage" @before-close="handleClose" :with-header="true" :title="$t('SysFrontUser.tag')">
    <template v-slot:content>
      <SysFrontUserTag
        :page-type="'create'"
...
// 表单页面
import SysFrontUserForm from './form.vue'
import SysFrontUserTag from './tag.vue'
export default defineComponent({
  name: 'SysFrontUser', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, DrawerPage, SysFrontUserForm, SysFrontUserTag }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, displaySysFrontUserHandle } = useSysFrontUserRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deleteSysFrontUserByIdHandle(params, () => {
        // 模板修改标记
        pgTableQuery(tableParams)
      })
    }
    ...
    // 标记
    function tagHandle(row: any) {
      drawerPageParams.value.userId = row.id
      drawerPageParams.value.tagIds = row.tagIds
      showDrawer()
    }
