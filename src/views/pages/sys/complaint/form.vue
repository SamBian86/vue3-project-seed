      ...
      <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('SysComplaint.complaintRemarks')" prop="complaintRemarks">
          <el-input
            type="textarea"
            :rows="5"
            v-model="formData.complaintRemarks"
            :placeholder="$t('SysComplaint.complaintRemarksPlaceHolder')"
            clearable
          ></el-input>
        </el-form-item>
      </el-col>
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const { formData, updateSysComplaintHandle, formPageResetHandle } = useSysComplaintRepository() // 模板修改标记
    ...
    onMounted(() => {
      ...
      watch([() => props.pageType, () => props.pageParams], ([type, params], [prevType, prevParams]) => {
        ...
        if (type === 'update' || type === 'detail') {
          hideSkeleton()
          // getSysComplaintByIdHandle(params, hideSkeleton) // 模板修改标记
        }
        pageType.value = type
      })
    })
    ...
    // 校验
    const rules = {
      complaintRemarks: [{ required: true, message: t('SysComplaint.complaintRemarksPlaceHolder'), trigger: 'blur' }] // 模板修改标记
    }
