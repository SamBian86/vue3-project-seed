      ...
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
              ...
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="name">
                  <el-input
                    ...
                    :placeholder="$t('SysServerarea.namePlaceHolder')"
                    clearable
                  ></el-input>
                </el-form-item>
              </el-col>
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              <el-button
                type="primary"
                v-if="filterPermission('sys:serverarea:view')"
              ...
              <el-button
                type="primary"
                v-if="filterPermission('sys:serverarea:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('SysServerarea.name')"
            width="160"
          ></el-table-column>
          <el-table-column prop="provinceName" :show-overflow-tooltip="true" :label="$t('SysServerarea.provinceName')">
          ...
          <el-table-column
            prop="deptName"
            :show-overflow-tooltip="true"
            :label="$t('SysServerarea.deptName')"
            width="380"
          ></el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="200">
            <template #default="scope">
              ...
              <el-button
                ...
                v-if="filterPermission('sys:serverarea:view')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="goToCommunity(scope.row)"
              >
                {{ $t('SysServerarea.list') }}
              </el-button>
              <el-button
                type="text"
                v-if="filterPermission('sys:serverarea:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('sys:serverarea:delete')"
                :size="StyleEnum.BUTTON_SIZE"
...
// 表单页面
import SysServerareaForm from './form.vue'
export default defineComponent({
  name: 'SysServerarea', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, SysServerareaForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deleteSysServerareaByIdHandle } = useSysServerareaRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deleteSysServerareaByIdHandle(params, () => {
        // 模板修改标记
    ...
    // 跳转到社区列表
    function goToCommunity(row: any) {
      const { provinceCode, cityCode, areaCode, streetCode } = row
      routerPush('/sys/community', (path: any) => {
        router.push({ path, query: { serverAreaId: row.id } })
      })
    }
