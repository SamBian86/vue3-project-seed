  ...
  <el-row :gutter="StyleEnum.ROW_GUTTER">
      ...
      >
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('HospitalZoneInfo.zoneName')"
            min-width="100"
          ></el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="220">
            <template #default="scope">
              ...
              <el-button
                ...
                v-if="filterPermission('hospitalzone:info:view')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="dialogHandle('set', 'showDialog', scope.row)"
              >
                {{ $t('HospitalZoneInfo.set') }}
              </el-button>
              <el-button
                type="text"
                v-if="filterPermission('hospitalzone:info:view') && scope.$index !== 0"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
              ...
              <el-button
                type="text"
                v-if="filterPermission('hospitalzone:info:view') && scope.$index !== tableUsefulData.len - 1"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
                  exchangeHospitalZoneInfoHandle(
                    { id: scope.row.id, exchangeId: tableUsefulData.data[scope.$index + 1]['id'] },
    ...
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <DialogPage ref="dialogPage">
        ...
        <template #title>
          {{ $t('HospitalZoneInfo.pageTitle') }}
        </template>
      </DialogPage>
    </el-col>
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <DialogPage ref="setDialogPage">
        <template #body>
          <div class="component-skeleton-page">
            <HospitalZoneInfoSet
              :page-type="'update'"
              :page-params="dialogPageParams"
              @hide-dialog="dialogHandle('set', 'hideDialog')"
              @update-table="pgTableQuery"
            />
          </div>
        </template>
...
// 表单页面
import HospitalZoneInfoForm from './form.vue'
import HospitalZoneInfoSet from './set.vue'
export default defineComponent({
  name: 'HospitalZoneInfo', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, HospitalZoneInfoForm, HospitalZoneInfoSet }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deleteHospitalZoneInfoByIdHandle, exchangeHospitalZoneInfoHandle } = useHospitalZoneInfoRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deleteHospitalZoneInfoByIdHandle(params, () => {
        // 模板修改标记
        pgTableQuery(tableParams)
      })
    }
    ...
    const setDialogPage = ref(null)
    const setDialogComponent = useDialogPageComponent(setDialogPage)
    const dialogPageParams = ref({})
    function dialogHandle(name: any, action: any, row: any) {
      const dialogPageComponent = {
        set: setDialogComponent
      }
      if (row) {
