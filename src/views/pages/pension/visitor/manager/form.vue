  ...
  >
    <el-row :gutter="StyleEnum.ROW_GUTTER">
      <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('PensionVisitorManager.employeeId')" prop="employeeId">
          <el-select
            class="el-select-block"
            :size="StyleEnum.FORM_SIZE"
            v-model="formData.employeeId"
            :placeholder="$t('PensionVisitorManager.employeeIdPlaceHolder')"
            filterable
            clearable
          >
      ...
      <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('PensionVisitorManager.communityIds')" prop="communityIds">
          <el-select
            class="el-select-block"
            :size="StyleEnum.FORM_SIZE"
            v-model="formData.communityIds"
            :placeholder="$t('PensionVisitorManager.communityIdsPlaceHolder')"
            multiple
            filterable
            clearable
          >
...
// API封装
import usePensionVisitorManagerRepository from './usePensionVisitorManagerRepository' // 模板修改标记
...
export default defineComponent({
  name: 'PensionVisitorManagerForm', // 模板修改标记
  mixins: [formMixin],
  components: {},
  computed: {
  ...
  setup(props, { emit }) {
    ...
    const {
      ...
      formPageResetHandle
    } = usePensionVisitorManagerRepository() // 模板修改标记
    ...
    onMounted(() => {
      ...
      watch([() => props.pageType, () => props.pageParams], ([type, params], [prevType, prevParams]) => {
        ...
        if (type === 'update' || type === 'detail') {
          getPensionVisitorManagerByIdHandle(params, hideSkeleton) // 模板修改标记
        }
        pageType.value = type
      })
    })
    ...
    // 校验
    const rules = {
      employeeId: [{ required: true, message: t('PensionVisitorManager.employeeIdPlaceHolder'), trigger: 'change' }], // 模板修改标记
      communityIds: [{ required: true, message: t('PensionVisitorManager.communityIdsPlaceHolder'), trigger: 'change' }]
    }

    // 提交逻辑
    function submitHandle() {
      const cMethod = pageType.value === 'create' ? createPensionVisitorManagerHandle : updatePensionVisitorManagerHandle // 模板修改标记
