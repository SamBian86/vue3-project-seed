      ...
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              <!-- <el-button
                type="primary"
                v-if="filterPermission('pension:serverdiscountdetail:view')"
              ...
              <el-button
                type="primary"
                v-if="filterPermission('pension:serverdiscountdetail:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('PensionServerdiscountdetail.description')"
            width="300"
          ></el-table-column>
          <el-table-column
            prop="scalePrice"
            :show-overflow-tooltip="true"
            :label="$t('PensionServerdiscountdetail.scalePrice')"
          ></el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="120">
            <template #default="scope">
              <el-button
                type="text"
                v-if="filterPermission('pension:serverdiscountdetail:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:serverdiscountdetail:delete')"
                :size="StyleEnum.BUTTON_SIZE"
      ...
      <DialogPage ref="dialogPage">
        <template #body>
          <SkeletonPage ref="skeletonPage">
            <template v-slot:content>
              <PensionServerdiscountdetailForm
                ref="formPage"
        ...
        <template #title>
          {{ $t('PensionServerdiscountdetail.pageTitle') }}
        </template>
      </DialogPage>
    </el-col>
  </el-row>
...
// 表单页面
import PensionServerdiscountdetailForm from './form.vue'
export default defineComponent({
  name: 'PensionServerdiscountdetail', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, PensionServerdiscountdetailForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deletePensionServerdiscountdetailByIdHandle } = usePensionServerdiscountdetailRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deletePensionServerdiscountdetailByIdHandle(params, () => {
        // 模板修改标记
