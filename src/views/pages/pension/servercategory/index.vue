  ...
  <el-row :gutter="StyleEnum.ROW_GUTTER">
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
          ...
          <el-form ref="searchForm" :model="tableParams" label-position="left" @keydown.enter.native="pgTableQuery(tableParams)">
            <el-row :gutter="StyleEnum.ROW_GUTTER">
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="putOn">
                  <el-select
                    class="el-select-block"
                    :size="StyleEnum.FORM_SIZE"
                    v-model="tableParams.putOn"
                    :placeholder="$t('PensionServercategory.putOn')"
                    clearable
                  >
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              <el-button
                type="primary"
                v-if="filterPermission('pension:servercategory:view')"
              ...
              <el-button
                type="primary"
                v-if="filterPermission('pension:servercategory:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column prop="photo" :show-overflow-tooltip="true" :label="$t('PensionServercategory.photo')">
          ...
          <el-table-column
            ...
            :label="$t('PensionServercategory.categoryName')"
            width="160"
          ></el-table-column>
          <el-table-column prop="putOn" :show-overflow-tooltip="true" :label="$t('PensionServercategory.putOn')" width="160">
            <template #default="scope">
              <el-tag v-if="scope.row.putOn === 0" :size="StyleEnum.TAG_SIZE" type="warning">
                {{ scope.row.putOnName }}
              </el-tag>
              <el-tag v-if="scope.row.putOn === 1" :size="StyleEnum.TAG_SIZE" type="success">
                {{ scope.row.putOnName }}
              </el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="180">
            <template #default="scope">
              <el-button
                type="text"
                v-if="filterPermission('pension:servercategory:display:update')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
                  displayPensionServercategoryHandle({ id: scope.row.id, display: scope.row.putOn === 1 ? 0 : 1 }, () => {
                    pgTableQuery(tableParams)
                  })
                "
              >
                {{ scope.row.putOn === 1 ? $t('table.off') : $t('table.on') }}
              </el-button>

              <el-button
                type="text"
                v-if="filterPermission('pension:servercategory:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:servercategory:delete')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:server:view')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="serverListHandle(scope.row)"
              >
                {{ $t('PensionServercategory.serverList') }}
              </el-button>
            </template>
          </el-table-column>
        </template>
    ...
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <DialogPage ref="dialogPage">
        ...
        <template #title>
          {{ $t('PensionServercategory.pageTitle') }}
        </template>
      </DialogPage>
    </el-col>
  </el-row>
...
// 表单页面
import PensionServercategoryForm from './form.vue'
export default defineComponent({
  name: 'PensionServercategory', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, PensionServercategoryForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deletePensionServercategoryByIdHandle, displayPensionServercategoryHandle } =
    ...
    function deleteHandle(params: any) {
      deletePensionServercategoryByIdHandle(params, () => {
        // 模板修改标记
        pgTableQuery(tableParams)
      })
    }
    ...
    const putOnItems = [
      {
        label: t('PensionServercategory.display0'),
        value: 0
      },
      {
        label: t('PensionServercategory.display1'),
        value: 1
      }
    ]
    ...
    // 服务列表跳转
    function serverListHandle(row: any) {
      routerPush('/pension/server', (path: any) => {
        router.push({ path, query: { serverCategoryId: row.id } })
      })
    }
