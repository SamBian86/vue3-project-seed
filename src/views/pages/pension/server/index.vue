      ...
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
        <template v-slot:search>
          <el-form ref="searchForm" :model="tableParams" label-position="left" @keydown.enter.native="pgTableQuery(tableParams)">
            <el-row :gutter="StyleEnum.ROW_GUTTER">
              ...
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="display">
                  <el-select
                    ...
                    :placeholder="$t('PensionServer.display')"
                    clearable
                  >
                    <el-option
              ...
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="serverNameOr">
                  <el-input
                    :size="StyleEnum.INPUT_SIZE"
                    v-model="tableParams.serverNameOr"
                    :placeholder="$t('PensionServer.serverNameOr')"
                    clearable
                  ></el-input>
                </el-form-item>
              </el-col>
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              ...
              <el-button
                type="primary"
                v-if="filterPermission('pension:server:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('PensionServer.serverCategoryName')"
            width="100"
          ></el-table-column>
          <el-table-column
            prop="name"
            :show-overflow-tooltip="true"
            :label="$t('PensionServer.name')"
            min-width="300"
          ></el-table-column>
          <el-table-column
            prop="businessName"
            :show-overflow-tooltip="true"
            :label="$t('PensionServer.businessName')"
            min-width="200"
          ></el-table-column>
          <el-table-column
            prop="typeName"
            :show-overflow-tooltip="true"
            :label="$t('PensionServer.typeName')"
            width="100"
          ></el-table-column>
          <el-table-column prop="display" :show-overflow-tooltip="true" :label="$t('PensionServer.display')" width="100">
            <template #default="scope">
              <el-tag v-if="scope.row.display === 0" :size="StyleEnum.TAG_SIZE" type="warning">
                {{ $t('PensionServer.display0') }}
              </el-tag>
              <el-tag v-if="scope.row.display === 1" :size="StyleEnum.TAG_SIZE" type="success">
                {{ $t('PensionServer.display1') }}
              </el-tag>
            </template>
          </el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="220">
            <template #default="scope">
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:server:display:update')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
                  displayPensionServerHandle({ id: scope.row.id, display: scope.row.display === 1 ? 0 : 1 }, () => {
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:server:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:server:delete')"
                :size="StyleEnum.BUTTON_SIZE"
...
// 表单页面
import PensionServerForm from './form.vue'
export default defineComponent({
  name: 'PensionServer', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, PensionServerForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    const tableParams = reactive({
      // 模板修改标记 serverCategoryId display type serverNameOr
      serverCategoryId: query.serverCategoryId ? query.serverCategoryId : ''
    })
    // 重置方法
    ...
    // API相关
    const { getPageHandle, deletePensionServerByIdHandle, displayPensionServerHandle } = usePensionServerRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deletePensionServerByIdHandle(params, () => {
        // 模板修改标记
        pgTableQuery(tableParams)
      })
    }
