  ...
  >
    <el-row :gutter="StyleEnum.ROW_GUTTER">
      <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('PensionBuilding.communityCode')" prop="communityCode">
          <el-select
            class="el-select-block"
            :size="StyleEnum.FORM_SIZE"
            v-model="formData.communityCode"
            :placeholder="$t('PensionBuilding.communityCodePlaceHolder')"
            filterable
            clearable
          >
      ...
      <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('PensionBuilding.buildingNumber')" prop="buildingNumber">
          <el-input
            v-model="formData.buildingNumber"
            :placeholder="$t('PensionBuilding.buildingNumberPlaceHolder')"
            clearable
          ></el-input>
        </el-form-item>
      </el-col>
...
export default defineComponent({
  name: 'PensionBuildingForm', // 模板修改标记
  mixins: [formMixin],
  components: {},
  computed: {
  ...
  setup(props, { emit }) {
    ...
    const {
      ...
      formPageResetHandle
    } = usePensionBuildingRepository() // 模板修改标记
    ...
    onMounted(() => {
      ...
      watch([() => props.pageType, () => props.pageParams], ([type, params], [prevType, prevParams]) => {
        ...
        if (type === 'update' || type === 'detail') {
          getPensionBuildingByIdHandle(params, hideSkeleton) // 模板修改标记
        }
        pageType.value = type
      })
    })
    ...
    // 校验
    const rules = {
      communityCode: [{ required: true, message: t('PensionBuilding.communityCodePlaceHolder'), trigger: 'change' }], // 模板修改标记
      buildingNumber: [{ required: true, message: t('PensionBuilding.buildingNumberPlaceHolder'), trigger: 'blur' }] // 模板修改标记
    }

    // 提交逻辑
    function submitHandle() {
      const cMethod = pageType.value === 'create' ? createPensionBuildingHandle : updatePensionBuildingHandle // 模板修改标记
