    ...
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      ...
      >
              ...
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="toolName">
                  <el-input
                    ...
                    v-model="tableParams.toolName"
                    :placeholder="$t('PensionTool.toolNamePlaceHolder')"
                    clearable
                  ></el-input>
                </el-form-item>
              </el-col>
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="toolType">
                  <el-select
                    class="el-select-block"
                    :size="StyleEnum.FORM_SIZE"
                    v-model="tableParams.toolType"
                    :placeholder="$t('PensionTool.toolTypePlaceHolder')"
                    filterable
                    clearable
                  >
                    <el-option
                      v-for="item in getDictByType('toolType')"
                      :key="item.dictValue"
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              <el-button
                type="primary"
                v-if="filterPermission('pension:tool:view')"
              ...
              <el-button
                type="primary"
                v-if="filterPermission('pension:tool:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column prop="toolIcon" :show-overflow-tooltip="true" :label="$t('PensionTool.toolIcon')" width="140">
            <template #default="scope">
              <el-image
                style="width: 50px; height: 50px"
                :fit="'cover'"
                :src="scope.row.toolIcon"
                :preview-src-list="[scope.row.toolIcon]"
              ></el-image>
            </template>
          </el-table-column>
          <el-table-column
            ...
            :label="$t('PensionTool.toolName')"
            width="140"
          ></el-table-column>
          <el-table-column
            prop="toolTypeName"
            :show-overflow-tooltip="true"
            :label="$t('PensionTool.toolTypeName')"
            width="80"
          ></el-table-column>
          <el-table-column
            prop="toolUrl"
            :show-overflow-tooltip="true"
            :label="$t('PensionTool.toolUrl')"
            min-width="240"
          ></el-table-column>
          <el-table-column prop="isHot" :show-overflow-tooltip="true" :label="$t('PensionTool.isHot')" width="100">
            <template #default="scope">
              <el-tag v-if="scope.row.isHot === 0" :size="StyleEnum.TAG_SIZE" type="success">
                {{ $t('PensionTool.isHot0') }}
              </el-tag>
              <el-tag v-if="scope.row.isHot === 1" :size="StyleEnum.TAG_SIZE" type="warning">
                {{ $t('PensionTool.isHot1') }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="220">
            <template #default="scope">
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:tool:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:tool:delete')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:tool:view') && scope.$index !== 0"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
              ...
              <el-button
                type="text"
                v-if="filterPermission('pension:tool:view') && scope.$index !== tableUsefulData.len - 1"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
                  exchangePensionToolHandle(
                    { id: scope.row.id, exchangeId: tableUsefulData.data[scope.$index + 1]['id'] },
...
// 表单页面
import PensionToolForm from './form.vue'
export default defineComponent({
  name: 'PensionTool', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, PensionToolForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deletePensionToolByIdHandle, exchangePensionToolHandle } = usePensionToolRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deletePensionToolByIdHandle(params, () => {
        // 模板修改标记
        pgTableQuery(tableParams)
      })
    }
