<template>
  <el-row :gutter="StyleEnum.ROW_GUTTER">
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <PgTable :table-method="getAgedActivitySignitemHandle" :table-params="tableParams" ref="pgTable">
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              ...
              <el-button
                type="primary"
                v-if="filterPermission('aged:activity:export')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="exportActionHandle"
              >
                {{ $t('table.export') }}
              </el-button>
            </el-col>
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('AgedActivity.name')"
            width="200"
          ></el-table-column>
          <el-table-column
            prop="mobile"
            :show-overflow-tooltip="true"
            :label="$t('AgedActivity.mobile')"
            min-width="160"
          ></el-table-column>
          <el-table-column
            prop="createTime"
            :show-overflow-tooltip="true"
            :label="$t('AgedActivity.createTime1')"
            width="160"
          ></el-table-column>
...
export default defineComponent({
  ...
  setup(props) {
    ...
    const tableParams = ref({
      // 模板修改标记
      activityId: props.pageParams.id,
      excelType: 2
    })
    ...
    // API相关
    const { exportHandle, getAgedActivitySignitemHandle } = useAgedActivityRepository() // 模板修改标记
    ...
    return {
      ...
      // API
      exportActionHandle,
      getAgedActivitySignitemHandle // 模板修改标记 获取分页数据
    }
  }
})
</script>
