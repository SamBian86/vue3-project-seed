      ...
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
              ...
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="name">
                  <el-input
                    ...
                    :placeholder="$t('AgedLinkaddress.namePlaceHolder')"
                    clearable
                  ></el-input>
                </el-form-item>
              </el-col>
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('AgedLinkaddress.name')"
            width="160"
          ></el-table-column>
          <el-table-column
            prop="mobile"
            :show-overflow-tooltip="true"
            :label="$t('AgedLinkaddress.mobile')"
            width="160"
          ></el-table-column>
          <el-table-column
            prop="communityName"
            :show-overflow-tooltip="true"
            :label="$t('AgedLinkaddress.communityName')"
            width="160"
          ></el-table-column>
          <el-table-column prop="address" :show-overflow-tooltip="true" :label="$t('AgedLinkaddress.address')"></el-table-column>
          <el-table-column
            prop="defaultAddress"
            :show-overflow-tooltip="true"
            :label="$t('AgedLinkaddress.defaultAddressCol')"
            width="100"
          >
            <template #default="scope">
              <el-tag v-if="scope.row.defaultAddress" :size="StyleEnum.TAG_SIZE" type="success">
                {{ $t('AgedLinkaddress.defaultAddressTrue') }}
              </el-tag>
              <el-tag v-if="!scope.row.defaultAddress" :size="StyleEnum.TAG_SIZE" type="warning">
                {{ $t('AgedLinkaddress.defaultAddressFalse') }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column
            prop="createDate"
            :show-overflow-tooltip="true"
            :label="$t('AgedLinkaddress.createDate')"
            width="160"
          ></el-table-column>
...
// 表单页面
import AgedLinkaddressForm from './form.vue'
export default defineComponent({
  name: 'AgedLinkaddress', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, AgedLinkaddressForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    const tableParams = reactive({
      // 模板修改标记
      userId: props.pageParams.userId
    })
    // 重置方法
    ...
    // API相关
    const { getPageHandle, deleteAgedLinkaddressByIdHandle } = useAgedLinkaddressRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deleteAgedLinkaddressByIdHandle(params, () => {
        // 模板修改标记
