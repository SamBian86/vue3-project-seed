  ...
  >
    <el-row :gutter="StyleEnum.ROW_GUTTER">
      <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('VolunteerExchangeserver.serverId')" prop="serverId">
          <el-select
            class="el-select-block"
            :size="StyleEnum.FORM_SIZE"
            v-model="formData.serverId"
            :placeholder="$t('VolunteerExchangeserver.serverIdPlaceHolder')"
            filterable
            clearable
          >
            <el-option v-for="(item, index) in pensionServerListAll" :key="index" :label="item" :value="index"></el-option>
          </el-select>
        </el-form-item>
      </el-col>
...
export default defineComponent({
  name: 'VolunteerExchangeserverForm', // 模板修改标记
  mixins: [formMixin],
  components: {},
  computed: {
  ...
  setup(props, { emit }) {
    ...
    const { pensionServerListAll, getPensionServerListAllHandle } = usePensionServerRepository()
    ...
    onMounted(() => {
      ...
      watch([() => props.pageType, () => props.pageParams], ([type, params], [prevType, prevParams]) => {
        ...
        if (type === 'update' || type === 'detail') {
          getVolunteerExchangeserverByIdHandle(params, hideSkeleton) // 模板修改标记
        }
        pageType.value = type
      })
    })
    ...
    // 校验
    const rules = {
      serverId: [{ required: true, message: t('VolunteerExchangeserver.serverIdPlaceHolder'), trigger: 'change' }] // 模板修改标记
    }

    // 提交逻辑
    function submitHandle() {
      const cMethod = pageType.value === 'create' ? createVolunteerExchangeserverHandle : updateVolunteerExchangeserverHandle // 模板修改标记
