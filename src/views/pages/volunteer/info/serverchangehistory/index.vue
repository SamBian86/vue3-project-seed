    ...
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
        <template v-slot:search>
          ...
          <b style="margin-right: 30px">累计时长:{{ volunteerInfo.totalTime }}小时</b>
          <b>可兑换时长:{{ volunteerInfo.availableTime }}小时</b>
        </template>
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              ...
              <el-button
                type="primary"
                v-if="filterPermission('volunteer:serverchangehistory:save')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="openHandle('create')"
              >
                {{ $t('VolunteerServerchangehistory.create') }}
              </el-button>
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('VolunteerServerchangehistory.createDate')"
            width="160"
          ></el-table-column>
          <el-table-column
            prop="sourceTypeName"
            :show-overflow-tooltip="true"
            :label="$t('VolunteerServerchangehistory.sourceTypeName')"
            width="140"
          ></el-table-column>
          <el-table-column
            prop="changeCount"
            :show-overflow-tooltip="true"
            :label="$t('VolunteerServerchangehistory.changeCount')"
            width="140"
          ></el-table-column>
          <el-table-column
            prop="operateNote"
            :show-overflow-tooltip="true"
            :label="$t('VolunteerServerchangehistory.operateNote')"
            min-width="200"
          ></el-table-column>
      ...
      <DialogPage ref="dialogPage">
        <template #body>
          <SkeletonPage ref="skeletonPage">
            <template v-slot:content>
              <VolunteerServerchangehistoryForm
                ref="formPage"
        ...
        <template #title>
          {{ $t('VolunteerServerchangehistory.pageTitle') }}
        </template>
      </DialogPage>
    </el-col>
  </el-row>
...
// 表单页面
import VolunteerServerchangehistoryForm from './form.vue'
export default defineComponent({
  name: 'VolunteerServerchangehistory', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, VolunteerServerchangehistoryForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    //   deleteVolunteerServerchangehistoryByIdHandle(params, () => {
    //     // 模板修改标记
