    ...
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
        <template v-slot:search>
          ...
          <b style="margin-right: 30px">总积分:{{ volunteerInfo.totalPoint }}</b>
          <b>可用积分:{{ volunteerInfo.availablePoint }}</b>
        </template>
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              ...
              <el-button
                type="primary"
                v-if="filterPermission('volunteer:integralchangehistory:save')"
                :size="StyleEnum.BUTTON_SIZE"
                @click="openHandle('create')"
              >
                {{ $t('VolunteerIntegralchangehistory.create') }}
              </el-button>
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('VolunteerIntegralchangehistory.createDate')"
            width="160"
          ></el-table-column>
          <el-table-column
            prop="sourceTypeName"
            :show-overflow-tooltip="true"
            :label="$t('VolunteerIntegralchangehistory.sourceTypeName')"
            width="140"
          ></el-table-column>
          <el-table-column
            prop="changeCount"
            :show-overflow-tooltip="true"
            :label="$t('VolunteerIntegralchangehistory.changeCount')"
            width="140"
          ></el-table-column>
          <el-table-column
            prop="operateNote"
            :show-overflow-tooltip="true"
            :label="$t('VolunteerIntegralchangehistory.operateNote')"
            min-width="200"
          ></el-table-column>
          ...
          <!-- <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="120">
            <template #default="scope">
              <el-button
                type="text"
                v-if="filterPermission('volunteer:integralchangehistory:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('volunteer:integralchangehistory:delete')"
                :size="StyleEnum.BUTTON_SIZE"
      ...
      <DialogPage ref="dialogPage">
        <template #body>
          <SkeletonPage ref="skeletonPage">
            <template v-slot:content>
              <VolunteerIntegralchangehistoryForm
                ref="formPage"
        ...
        <template #title>
          {{ $t('VolunteerIntegralchangehistory.pageTitle') }}
        </template>
      </DialogPage>
    </el-col>
  </el-row>
...
// API封装
import useVolunteerIntegralchangehistoryRepository from '../../integralchangehistory/useVolunteerIntegralchangehistoryRepository' // 模板修改标记
import useVolunteerInfoRepository from '../../info/useVolunteerInfoRepository'
// mixin
import tableMixin from '/@/mixins/tableMixin'
// 表单页面
import VolunteerIntegralchangehistoryForm from './form.vue'
export default defineComponent({
  name: 'VolunteerIntegralchangehistory', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, VolunteerIntegralchangehistoryForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    const tableParams = reactive({
      // 模板修改标记
      volunteerId: props.pageParams.id
    })
    // 重置方法
    ...
    // API相关
    const { getPageHandle } = useVolunteerIntegralchangehistoryRepository() // 模板修改标记
    const { getVolunteerInfoByIdHandle } = useVolunteerInfoRepository()
    getVolunteerInfoByIdHandle({ id: props.pageParams.id }).then((response) => {
      volunteerInfo.value = response
    })
    ...
    function createHandle() {
      ...
      formPageParams.value.volunteerId = props.pageParams.id
    }
    // 修改
    ...
    //   deleteVolunteerIntegralchangehistoryByIdHandle(params, () => {
    //     // 模板修改标记
