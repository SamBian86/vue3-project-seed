      ...
      <PgTable :table-method="getPageHandle" :table-params="tableParams" ref="pgTable">
              ...
              <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="4" :lg="4" :xl="4">
                <el-form-item prop="name">
                  <el-input
                    ...
                    :placeholder="$t('CardOperationinfo.namePlaceHolder')"
                    clearable
                  ></el-input>
                </el-form-item>
              </el-col>
        ...
        <template v-slot:button>
          <el-row :gutter="StyleEnum.ROW_GUTTER">
            <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
              <el-button
                type="primary"
                v-if="filterPermission('card:operationinfo:view')"
              ...
              <el-button
                type="primary"
                v-if="filterPermission('card:operationinfo:save')"
                :size="StyleEnum.BUTTON_SIZE"
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('CardOperationinfo.operationName')"
            width="300"
          ></el-table-column>
          <el-table-column prop="operationStatus" :show-overflow-tooltip="true" :label="$t('CardOperationinfo.operationStatus')">
            <template #default="scope">
              <el-tag v-if="scope.row.operationStatus === 0" :size="StyleEnum.TAG_SIZE" type="success">
                {{ $t('CardOperationinfo.operationStatus0') }}
              </el-tag>
              <el-tag v-if="scope.row.operationStatus === 1" :size="StyleEnum.TAG_SIZE" type="success">
                {{ $t('CardOperationinfo.operationStatus1') }}
              </el-tag>
            </template>
          </el-table-column>
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="120">
            <template #default="scope">
              <el-button
                type="text"
                v-if="filterPermission('card:operationinfo:update')"
                :size="StyleEnum.BUTTON_SIZE"
              ...
              <el-button
                type="text"
                v-if="filterPermission('card:operationinfo:delete')"
                :size="StyleEnum.BUTTON_SIZE"
...
// API封装
import useCardOperationinfoRepository from './useCardOperationinfoRepository' // 模板修改标记
// mixin
import tableMixin from '/@/mixins/tableMixin'
// 表单页面
import CardOperationinfoForm from './form.vue'
export default defineComponent({
  name: 'CardOperationinfo', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, CardOperationinfoForm }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deleteCardOperationinfoByIdHandle } = useCardOperationinfoRepository() // 模板修改标记
    ...
    function deleteHandle(params: any) {
      deleteCardOperationinfoByIdHandle(params, () => {
        // 模板修改标记
