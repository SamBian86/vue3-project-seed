  ...
  >
    <el-row :gutter="StyleEnum.ROW_GUTTER">
      <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
        <el-form-item :label="$t('HospitalInfo.introduce')" prop="introduce">
          <Tinymce
            ref="tinymce"
            :data="formData.introduce"
            :height="'400'"
            :placeholder="$t('HospitalInfo.introducePlaceHolder')"
...
export default defineComponent({
  ...
  setup(props, { emit }) {
    ...
    const { formData, updateHospitalInfoIntroduceByIdHandle, getHospitalInfoByIdHandle, formPageResetHandle } =
      useHospitalInfoRepository() // 模板修改标记
    const ruleForm = ref(null)
    const { formPageSubmitHandle } = useFormPageComponent(ruleForm)

    getHospitalInfoByIdHandle(props.pageParams).then((response) => {
      introduce.value = response.introduce
    }) // 模板修改标记
    ...
    // 校验
    const rules = {
      introduce: [{ required: true, message: t('HospitalInfo.introducePlaceHolder'), trigger: 'blur' }] // 模板修改标记
    }
    ...
    function submitHandle() {
      const cMethod = updateHospitalInfoIntroduceByIdHandle // 模板修改标记
