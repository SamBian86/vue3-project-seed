  ...
  <el-row :gutter="StyleEnum.ROW_GUTTER">
      ...
      >
        ...
        <template v-slot:content>
          <el-table-column
            ...
            :label="$t('HospitaldepartmentInfo.deptName')"
            min-width="100"
          ></el-table-column>
          ...
          <el-table-column :show-overflow-tooltip="true" :label="$t('table.handle')" width="220">
            <template #default="scope">
              ...
              >
                {{ $t('HospitaldepartmentInfo.introduce') }}
              </el-button>
              <el-button
                type="text"
                v-if="filterPermission('hospitaldepartment:info:view') && scope.$index !== 0"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
              ...
              <el-button
                type="text"
                v-if="filterPermission('hospitaldepartment:info:view') && scope.$index !== tableUsefulData.len - 1"
                :size="StyleEnum.BUTTON_SIZE"
                @click="
                  exchangeHospitaldepartmentInfoHandle(
                    { id: scope.row.id, exchangeId: tableUsefulData.data[scope.$index + 1]['id'] },
    ...
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <DialogPage ref="dialogPage">
        ...
        <template #title>
          {{ $t('HospitaldepartmentInfo.pageTitle') }}
        </template>
      </DialogPage>
    </el-col>
    <el-col :xs="StyleEnum.COL_XS" :sm="StyleEnum.COL_SM" :md="24" :lg="24" :xl="24">
      <DialogPage ref="introduceDialogPage">
        <template #body>
          <div class="component-skeleton-page">
            <HospitaldepartmentInfoIntroduce
              :page-type="'update'"
...
// API封装
import useHospitaldepartmentInfoRepository from './useHospitaldepartmentInfoRepository' // 模板修改标记
// mixin
import tableMixin from '/@/mixins/tableMixin'
// 表单页面
import HospitaldepartmentInfoForm from './form.vue'
import HospitaldepartmentInfoIntroduce from './introduce.vue'

export default defineComponent({
  name: 'HospitaldepartmentInfo', // 模板修改标记
  mixins: [tableMixin],
  components: { PgTable, SkeletonPage, DialogPage, HospitaldepartmentInfoForm, HospitaldepartmentInfoIntroduce }, // 模板修改标记
  computed: {
  ...
  setup(props) {
    ...
    // API相关
    const { getPageHandle, deleteHospitaldepartmentInfoByIdHandle, exchangeHospitaldepartmentInfoHandle } =
      useHospitaldepartmentInfoRepository() // 模板修改标记
    ...
    function createHandle() {
      ...
      formPageParams.value.hospitalId = props.pageParams.id
    }
    // 修改
    ...
    function deleteHandle(params: any) {
      deleteHospitaldepartmentInfoByIdHandle(params, () => {
        // 模板修改标记
        pgTableQuery(tableParams)
      })
    }
    ...
    const introduceDialogPage = ref(null)
    const dialogPageParams = ref('')
    const introduceDialogComponent = useDialogPageComponent(introduceDialogPage)

    function dialogHandle(name: any, action: any, row: any) {
      const dialogPageComponent = {
        introduce: introduceDialogComponent
      }
      if (row) {
